...
export class RelevanceScorer {
  ...
  async buildEmbeddings(filePaths: string[]): Promise<void> {
    ...
    console.log(
      `Generated embeddings for ${this.symbolEmbeddings.size} symbols`,
    );
  }
    ...
    for (const [key, embedding] of this.symbolEmbeddings) {
      ...
      if (similarity > 0.1) {
        results.push({
          ...
          features: {
            structural: this.calculateStructuralSimilarity(
            ...
            contextual: this.calculateContextualSimilarity(
              targetEmbedding.features,
              embedding.features,
            ),
            usage: this.calculateUsageSimilarity(
              targetEmbedding.features,
              embedding.features,
            ),
          },
  ...
  private async extractFileFeatures(
    filePath: string,
    ...
    for (const [symbol, symbolInfo] of context.symbols) {
      const symbolFeatures = this.extractSymbolFeatures(
        symbol,
        symbolInfo,
        context,
      );
  ...
  private filterCandidates(
    filters?: RelevanceQuery["filters"],
  ): Map<string, SymbolEmbedding> {
    ...
    for (const [key, embedding] of this.symbolEmbeddings) {
      ...
      if (
        filters.symbolTypes &&
        !filters.symbolTypes.includes(embedding.features.type)
      ) {
        include = false;
      }
      ...
      if (
        filters.files &&
        !filters.files.some((f) => embedding.file.includes(f))
      ) {
        include = false;
      }
      ...
      if (
        ...
        embedding.features.complexity > filters.maxComplexity
      ) {
        include = false;
      }

      if (
        filters.isPublicAPI !== undefined &&
        embedding.features.isPublicAPI !== filters.isPublicAPI
      ) {
        include = false;
      }
  ...
  private calculateSimilarity(
    ...
    const semantic = this.calculateCosineSimilarity(
      queryEmbedding,
      symbolEmbedding.embedding,
    );
    const structural = this.calculateStructuralRelevance(
      query,
      symbolEmbedding.features,
    );
    const contextual = this.calculateContextualRelevance(
      query,
      symbolEmbedding,
    );
    ...
    return {
      ...
      explanation: query.options?.includeExplanation
        ? this.generateRelevanceExplanation(query, symbolEmbedding, {
            semantic,
            structural,
            contextual,
            usage,
  ...
  private calculateContextualRelevance(
    ...
    if (
      ...
      embedding.file === query.context.currentFile
    ) {
  ...
  private applyContextBoosting(
    ...
    context?: RelevanceQuery["context"],
  ): void {
    if (!context) return;
  ...
  private generateSimilarityExplanation(
    ...
    if (
      Math.abs(target.features.complexity - candidate.features.complexity) < 2
    ) {
  ...
  private calculateUsageSimilarity(
    a: SymbolFeatures,
    b: SymbolFeatures,
  ): number {
  ...
  getEmbeddingStats(): {
    ...
    const averageEmbeddingMagnitude =
      totalEmbeddings > 0 ? totalMagnitude / totalEmbeddings : 0;
