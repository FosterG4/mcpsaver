...
export class EmbeddingGenerator {
  ...
  async generateSymbolEmbedding(
    ...
    if (this.codeModel) {
      embedding = this.generateCodeEmbedding(tokens, structure, semantics, features);
  ...
  ): Promise<Array<{ id: string; similarity: number; metrics: SimilarityMetrics }>> {
    const results: Array<{ id: string; similarity: number; metrics: SimilarityMetrics }> = [];
  ...
  async clusterEmbeddings(
    ...
    console.log(`Clustering ${embeddings.size} embeddings into ${numClusters} clusters...`);
    
    const points = Array.from(embeddings.values()).map(e => new Float32Array(e.values));
    ...
    for (let k = 0; k < numClusters; k++) {
      ...
      const cohesion = this.calculateCohesion(points, assignments, k, centroids[k]);
      const separation = this.calculateSeparation(centroids, k);
      const silhouetteScore = this.calculateSilhouetteScore(points, assignments, k);
  ...
  ): Promise<{ reducedEmbeddings: Map<string, EmbeddingVector>; reduction: DimensionalityReduction }> {
    console.log(`Reducing dimensionality from ${this.dimensions} to ${targetDimensions} using ${method.toUpperCase()}...`);
  ...
  async exportModels(outputPath: string): Promise<void> {
    const modelData = {
      word2vec: this.word2vecModel ? {
        ...
        embeddings: this.word2vecModel.embeddings.map(arr => Array.from(arr)),
      ...
      doc2vec: this.doc2vecModel ? {
        wordModel: {
          vocabulary: Array.from(this.doc2vecModel.wordModel.vocabulary.entries()),
          embeddings: this.doc2vecModel.wordModel.embeddings.map(arr => Array.from(arr)),
          dimensions: this.doc2vecModel.wordModel.dimensions
        },
        documentEmbeddings: Array.from(this.doc2vecModel.documentEmbeddings.entries())
      ...
      codeModel: this.codeModel ? {
        tokenEmbeddings: Array.from(this.codeModel.tokenEmbeddings.entries())
          .map(([token, emb]) => [token, Array.from(emb)]),
        structuralEmbeddings: Array.from(this.codeModel.structuralEmbeddings.entries())
          .map(([token, emb]) => [token, Array.from(emb)]),
        semanticEmbeddings: Array.from(this.codeModel.semanticEmbeddings.entries())
          .map(([token, emb]) => [token, Array.from(emb)]),
  ...
  async importModels(inputPath: string): Promise<void> {
    ...
    if (modelData.word2vec) {
      this.word2vecModel = {
        ...
        embeddings: modelData.word2vec.embeddings.map((arr: number[]) => new Float32Array(arr)),
    ...
    if (modelData.doc2vec) {
      this.doc2vecModel = {
        wordModel: {
          ...
          embeddings: modelData.doc2vec.wordModel.embeddings.map((arr: number[]) => new Float32Array(arr)),
        ...
        documentEmbeddings: new Map(
          modelData.doc2vec.documentEmbeddings.map(([id, emb]: [string, number[]]) => 
            [id, new Float32Array(emb)]
          )
        ),
    ...
    if (modelData.codeModel) {
      this.codeModel = {
        tokenEmbeddings: new Map(
          modelData.codeModel.tokenEmbeddings.map(([token, emb]: [string, number[]]) => 
            [token, new Float32Array(emb)]
          )
        ),
        structuralEmbeddings: new Map(
          modelData.codeModel.structuralEmbeddings.map(([token, emb]: [string, number[]]) => 
            [token, new Float32Array(emb)]
          )
        ),
        semanticEmbeddings: new Map(
          modelData.codeModel.semanticEmbeddings.map(([token, emb]: [string, number[]]) => 
            [token, new Float32Array(emb)]
          )
        ),
    ...
    for (let iter = 0; iter < iterations; iter++) {
      ...
      for (const sentence of corpus) {
        for (let i = 0; i < sentence.length; i++) {
          ...
          for (let j = Math.max(0, i - this.windowSize); j <= Math.min(sentence.length - 1, i + this.windowSize); j++) {
            if (i === j) continue;
            ...
            // Positive sample
            this.updateEmbeddings(embeddings, centerIndex, contextIndex, 1, learningRate);
            
            // Negative samples
            for (let k = 0; k < negativeSamples; k++) {
              const negativeIndex = Math.floor(Math.random() * embeddings.length);
              if (negativeIndex !== contextIndex) {
                this.updateEmbeddings(embeddings, centerIndex, negativeIndex, 0, learningRate);
              }
            }
          }
        }
  ...
  private generateCodeEmbedding(
    ...
    const tokenEmb = this.averageEmbeddings(tokens, this.codeModel.tokenEmbeddings);
    for (let i = 0; i < this.dimensions; i++) {
      embedding[i] += tokenEmb[i] * weights.token;
    }
    
    // Combine structural embeddings
    const structEmb = this.averageEmbeddings(structure, this.codeModel.structuralEmbeddings);
    for (let i = 0; i < this.dimensions; i++) {
      embedding[i] += structEmb[i] * weights.structural;
    }
    
    // Combine semantic embeddings
    const semEmb = this.averageEmbeddings(semantics, this.codeModel.semanticEmbeddings);
    for (let i = 0; i < this.dimensions; i++) {
  ...
  private async generateDocEmbedding(tokens: string[]): Promise<Float32Array> {
    ...
    return this.averageEmbeddings(tokens, this.doc2vecModel.wordModel.embeddings, this.doc2vecModel.wordModel.vocabulary);
  }
  ...
  private generateWordEmbedding(tokens: string[]): Float32Array {
    ...
    return this.averageEmbeddings(tokens, this.word2vecModel.embeddings, this.word2vecModel.vocabulary);
  }
  ...
  private addFeatureComponents(embedding: Float32Array, features: CombinedFeatures): void {
  ...
  private jaccardSimilarity(vec1: number[], vec2: number[]): number {
    const set1 = new Set(vec1.map((v, i) => v > 0 ? i : -1).filter(i => i >= 0));
    const set2 = new Set(vec2.map((v, i) => v > 0 ? i : -1).filter(i => i >= 0));
  ...
  private initializeCentroids(points: Float32Array[], numClusters: number): Float32Array[] {
  ...
  private findNearestCentroid(point: Float32Array, centroids: Float32Array[]): number {
    let minDistance = Infinity;
    let nearestIndex = 0;
  ...
  private calculateSeparation(centroids: Float32Array[], clusterIndex: number): number {
    let minDistance = Infinity;
  ...
  private calculateSilhouetteScore(
    ...
    for (const point of clusterPoints) {
      ...
      const b = this.averageNearestClusterDistance(point, points, assignments, clusterIndex);
  ...
  private averageIntraClusterDistance(point: Float32Array, clusterPoints: Float32Array[]): number {
    if (clusterPoints.length <= 1) return 0;
  ...
  private averageNearestClusterDistance(
    ...
    const otherClusters = new Set(assignments.filter(a => a !== currentCluster));
    let minAvgDistance = Infinity;
    
    for (const otherCluster of otherClusters) {
      const otherClusterPoints = allPoints.filter((_, i) => assignments[i] === otherCluster);
  ...
  private performPCA(
    points: Float32Array[],
    targetDimensions: number
  ): { reducedPoints: Float32Array[]; explainedVariance: number; components: Float32Array[] } {
  ...
  private performTSNE(points: Float32Array[], targetDimensions: number): Float32Array[] {
  ...
  private performUMAP(points: Float32Array[], targetDimensions: number): Float32Array[] {
