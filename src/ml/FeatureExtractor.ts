...
export class FeatureExtractor {
  ...
  constructor() {
    this.technicalTerms = new Set([
      'api', 'http', 'json', 'xml', 'sql', 'database', 'cache', 'queue',
      'thread', 'async', 'sync', 'promise', 'callback', 'event', 'stream',
      'buffer', 'socket', 'connection', 'session', 'token', 'auth', 'oauth',
      'encryption', 'hash', 'algorithm', 'data', 'model', 'view', 'controller',
      'service', 'repository', 'factory', 'builder', 'adapter', 'proxy',
      'decorator', 'observer', 'strategy', 'command', 'state', 'visitor'
    ]);
    
    this.domainTerms = new Map([
      ['web', ['html', 'css', 'dom', 'browser', 'client', 'server', 'request', 'response']],
      ['database', ['table', 'column', 'row', 'index', 'query', 'transaction', 'schema']],
      ['security', ['authentication', 'authorization', 'permission', 'role', 'user', 'password']],
      ['business', ['order', 'customer', 'product', 'invoice', 'payment', 'account']],
      ['ui', ['button', 'form', 'input', 'dialog', 'menu', 'layout', 'component']]
    ]);
  ...
  async extractFeatures(
    ...
    const contextFeatures = await this.extractContextFeatures(filePath, symbol, context);
    const usage = await this.extractUsageFeatures(symbol, context);
    const semantic = await this.extractSemanticFeatures(symbol, symbolInfo, context);
    
    // Calculate composite scores
    const relevanceScore = this.calculateRelevanceScore(text, code, contextFeatures, usage, semantic);
    const qualityScore = this.calculateQualityScore(code, semantic);
    const maintainabilityScore = this.calculateMaintainabilityScore(code, semantic);
   ...
   */
  async extractTextFeatures(symbol: string, symbolInfo: SymbolInfo): Promise<TextFeatures> {
    ...
    return {
      ...
      followsNamingConvention: this.followsNamingConvention(symbol, symbolInfo.type)
    };
  }
   ...
   */
  async extractCodeFeatures(symbolInfo: SymbolInfo, context: SemanticContext): Promise<CodeFeatures> {
   ...
   */
  async extractUsageFeatures(symbol: string, context: SemanticContext): Promise<UsageFeatures> {
  ...
  async extractSemanticFeatures(
    ...
    return {
      ...
      responsibilityFocus: this.calculateResponsibilityFocus(symbolInfo, context),
  ...
  private extractTechnicalTerms(tokens: string[]): string[] {
    return tokens.filter(token => this.technicalTerms.has(token.toLowerCase()));
  }

  private extractNounPhrases(tokens: string[]): string[] {
    // Simplified noun phrase extraction
    const nounIndicators = ['data', 'info', 'list', 'map', 'set', 'array', 'object', 'item'];
    return tokens.filter(token => nounIndicators.some(indicator => token.includes(indicator)));
  }

  private extractVerbPhrases(tokens: string[]): string[] {
    // Simplified verb phrase extraction
    const verbIndicators = ['get', 'set', 'create', 'delete', 'update', 'find', 'search', 'process', 'handle'];
    return tokens.filter(token => verbIndicators.some(indicator => token.startsWith(indicator)));
  }
  ...
  private isAcronym(symbol: string): boolean {
    return symbol.length <= 5 && symbol === symbol.toUpperCase() && /^[A-Z]+$/.test(symbol);
  }
  ...
  private calculateFanIn(symbolInfo: SymbolInfo, context: SemanticContext): number {
  ...
  private calculateInstability(symbolInfo: SymbolInfo, context: SemanticContext): number {
  ...
  private calculateCallFrequency(symbolInfo: SymbolInfo, context: SemanticContext): number {
  ...
  private detectCodeSmells(symbolInfo: SymbolInfo): string[] {
    ...
    if ((symbolInfo.dependencies?.length || 0) > 10) smells.push('feature-envy');
    
    return smells;
  }

  private detectDesignPatterns(symbolInfo: SymbolInfo, context: SemanticContext): string[] {
  ...
  private detectAntiPatterns(symbolInfo: SymbolInfo, context: SemanticContext): string[] {
    const antiPatterns: string[] = [];
    const name = symbolInfo.name || '';
    
    if (name.includes('God') || name.includes('Manager')) antiPatterns.push('god-object');
    if (name.includes('Util') || name.includes('Helper')) antiPatterns.push('utility-class');
    
    return antiPatterns;
  }

  private determineArchitecturalRole(symbolInfo: SymbolInfo, context: SemanticContext): string {
  ...
  private findSiblingSymbols(symbol: string, context: SemanticContext): string[] {
    // Would find symbols in the same scope/file
    return [];
  }

  private findParentSymbols(symbol: string, context: SemanticContext): string[] {
  ...
  private findChildSymbols(symbol: string, context: SemanticContext): string[] {
    ...
    for (const [otherSymbol, relationships] of context.relationships) {
      for (const rel of relationships) {
        if ((rel.type === 'extends' || rel.type === 'implements') && rel.target === symbol) {
  ...
  private findRelatedFiles(filePath: string, context: SemanticContext): string[] {
    // Would find files with related symbols
    return [];
  }

  // Semantic analysis methods
  private calculateAbstractionLevel(symbolInfo: SymbolInfo, context: SemanticContext): number {
  ...
  private calculateDomainSpecificity(symbol: string, symbolInfo: SymbolInfo): number {
    const tokens = this.tokenizeSymbolName(symbol);
  ...
  private calculateBusinessRelevance(symbol: string, symbolInfo: SymbolInfo): number {
    const tokens = this.tokenizeSymbolName(symbol);
    const businessTerms = ['order', 'customer', 'product', 'payment', 'account', 'user'];
    const hasBusinessTerms = tokens.some(token => businessTerms.includes(token.toLowerCase()));
    
    return hasBusinessTerms ? 0.8 : 0.2;
  }

  private calculateSemanticCohesion(symbolInfo: SymbolInfo, context: SemanticContext): number {
    // Would analyze how well the symbol's responsibilities are related
    return 0.7;
  }

  private calculateConceptualCoupling(symbolInfo: SymbolInfo, context: SemanticContext): number {
  ...
  private calculatePurposeClarity(symbol: string, symbolInfo: SymbolInfo): number {
    const tokens = this.tokenizeSymbolName(symbol);
  ...
  private calculateResponsibilityFocus(symbolInfo: SymbolInfo, context: SemanticContext): number {
    const dependencies = symbolInfo.dependencies?.length || 0;
    const lines = (symbolInfo.endLine || 0) - (symbolInfo.startLine || 0);
  ...
  private calculateSideEffectRisk(symbolInfo: SymbolInfo): number {
    const name = (symbolInfo.name || '').toLowerCase();
    const riskKeywords = ['delete', 'remove', 'clear', 'reset', 'modify', 'update'];
  ...
  private calculateDeprecationRisk(symbol: string, symbolInfo: SymbolInfo): number {
    const name = symbol.toLowerCase();
    const deprecationIndicators = ['legacy', 'old', 'deprecated', 'obsolete'];
    
    return deprecationIndicators.some(indicator => name.includes(indicator)) ? 0.9 : 0.1;
  }
  ...
  private calculateQualityScore(code: CodeFeatures, semantic: SemanticFeatures): number {
    ...
    return (maintainability + testCoverage + documentation + cohesion + focus) / 5;
  }

  private calculateMaintainabilityScore(code: CodeFeatures, semantic: SemanticFeatures): number {
  ...
  async extractBatchFeatures(
    symbols: Array<{ symbol: string; symbolInfo: SymbolInfo; filePath: string }>,
    context: SemanticContext
    ...
    for (const { symbol, symbolInfo, filePath } of symbols) {
      const symbolFeatures = await this.extractFeatures(symbol, symbolInfo, context, filePath);
  ...
  getFeatureWeights(): Record<string, number> {
    return {
      ...
      'semantic.businessRelevance': 0.10
    };
  }
