...
import type { CodeContext, ExtractContextOptions, DiffAnalysisOptions, ImportOptimizationOptions } from '../types/index.js';
...
export class CodeReferenceOptimizer {
  ...
  async optimizeImports(options: ImportOptimizationOptions): Promise<{
    ...
    const optimizedImports = await this.importAnalyzer.getMinimalImports(filePath, usedSymbols || []);
  ...
  private generateCacheKey(filePath: string, targetSymbols?: string[], includeImports?: boolean): string {
  ...
  private truncateToTokenLimit(context: CodeContext, maxTokens: number): CodeContext {
  ...
  private extractUsedSymbols(context: { code: string; symbols: string[] }): string[] {
  ...
  private calculateRelevanceScore(context: { symbols: string[] }, targetSymbols?: string[]): number {
  ...
  private generateMinimalUpdate(changes: Array<{
    type: 'added' | 'removed' | 'modified';
    symbol: string;
    code: string;
    lineNumber: number;
  ...
  private async extractSymbolsFromContent(content: string, filePath: string): Promise<Map<string, string>> {
  ...
  private extractSymbolCode(content: string, symbolName: string): string {
    ...
    const symbolRegex = new RegExp(`(class|function|interface|type|const|let|var)\\s+${symbolName}\\b`);
